...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    if (!user) {
      try {
        const userInfo = await this.getUserInfoWithJwt(sessionCookie ?? "");
        // Create OAuth user with temporary username
        const tempUsername = `oauth_${userInfo.openId.substring(0, 20)}`;
        await db.createUser(tempUsername, "", userInfo.name || undefined, userInfo.email || undefined);
        // Update with openId for reference
        const newUser = await db.getUserByUsername(tempUsername);
        if (newUser) {
          await db.upsertUser({
            id: newUser.id,
